"""RNN cores that decode from a bottleneck initialization."""
...
class GRUSequenceDecoder(SequenceDecoder):
    """Same pattern as :class:`LSTMSequenceDecoder` with a GRU core."""
    ...
    def __init__(
        ...
        self.vocab_size = vocab_size
        self.n_layers = n_layers
        self.cell = nn.GRU(d_model, d_model, num_layers=n_layers, batch_first=True)
        self.step_in = nn.Parameter(torch.zeros(1, 1, d_model))
        self.init_h = nn.Linear(z_dim, n_layers * d_model)
        self.head = nn.Linear(d_model, vocab_size)
